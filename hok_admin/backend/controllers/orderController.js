...
export const createOrder = async (req, res) => { try {
  ...
  for (const input of req.body.items) { const product = await Product.findOne({ $or: [{ productId: input.productId }, { _id: input.productId }] }); if (!product) return res.status(422).json({ success: false, message: `Product not found: ${input.productId}` }); calculatedItems.push(calculateProductLine(product, input)); }
