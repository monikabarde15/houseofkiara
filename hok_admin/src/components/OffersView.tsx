...
export default function OffersView({ offers, products, listers, currentAdminName = 'Soumya', loading = false, onUpdateOffer, onAddOffer }: OffersViewProps) {
  ...
  const [form, setForm] = useState({ customerName: '', source: 'Phone', email: '', phone: '', productName: '', assignedTo: currentAdminName, offerAmount: '', note: '' });
  ...
  const selectedProductPrice = selectedProduct ? (selectedProduct.listingPrice || selectedProduct.rentalPrice || selectedProduct.originalRetailPrice || 0) : 0;
  ...
  const addOffer = (event: React.FormEvent) => {
    ...
    const marketPrice = selectedProductPrice;
    onAddOffer({ id: `OFR-${Date.now().toString().slice(-6)}`, customerName: form.customerName, customerEmail: form.email, phone: form.phone, productName: form.productName, marketPrice, offerPrice, askPercentage: offerPrice ? Math.round((offerPrice / marketPrice) * 100) : 0, date: new Date().toISOString().slice(0, 10), status: offerPrice ? 'Pending' : 'Enquiry', channel: form.source, note: form.note, assignedTo: form.assignedTo });
  ...
  const exportOffers = () => {
    ...
    const rows = [
      ...
      ...visibleOffers.map((offer) => [offer.id, offer.productName, offer.marketPrice, offer.offerPrice || 'Enquiry only', offer.offerPrice ? `${offer.askPercentage}%` : '—', offer.customerName, offer.customerEmail, offer.phone || '', offer.channel || '', offer.status, offer.date, offer.note || '']),
    ];
  ...
  return <section className="offers-page">
    ...
    <div className="offer-table-card">
      <Toolbar status={selectedStatus} onStatus={(status) => { setSelectedStatus(status); setPage(1); }} onExport={exportOffers} onNew={() => { setForm({ customerName: '', source: 'Phone', email: '', phone: '', productName: '', assignedTo: currentAdminName, offerAmount: '', note: '' }); setShowNewOffer(true); }} />
      {showNewOffer && <NewOfferForm form={form} selectedProduct={selectedProduct} selectedProductPrice={selectedProductPrice} products={products} listers={listers} currentAdminName={currentAdminName} updateForm={updateForm} onSubmit={addOffer} onCancel={() => setShowNewOffer(false)} />}
...
function Log({ text, by, date }: { text: string; by: string; date?: string }) { return <div className="log-line"><span className="log-dot" /><div><small>{date ? new Date(date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })} · {by}</small><p>{text}</p></div></div>; }
